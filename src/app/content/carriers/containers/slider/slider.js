...
function Slider() {
  ...
  <>
    ...
    <div className={style.carriers}>
      ...
      <img src={(selectedRoute && carrierStore.findCarrierById(selectedRoute.carrierId) ? carrierStore.findCarrierById(selectedRoute.carrierId).pic : `https://avatars.dicebear.com/v2/identicon/unknown.svg`)} className={(uiStoreCarriers.selectedRoute && uiStoreCarriers.selectedRoute.status === "completed" ? (`${style.currentCarrier} ${style.slideCompletedCarrier}`) : uiStoreCarriers.selectedRoute && uiStoreCarriers.selectedRoute.status === "awaiting" ? (`${style.currentCarrier} ${style.sliderAwaitingCarrier}`) : uiStoreCarriers.selectedRoute && uiStoreCarriers.selectedRoute.status === "happening" ?(`${style.currentCarrier} ${style.sliderCurrentCarrierBorder}`):null )} alt="person"></img>
      <img src={routeposition != "" && routeArray[routeposition - 1]  && carrierStore.findCarrierById(routeArray[routeposition - 1].carrierId) ? carrierStore.findCarrierById(routeArray[routeposition - 1].carrierId).pic : `https://avatars.dicebear.com/v2/identicon/unknown.svg`  } className={style.nextCarrier} alt="person"></img>
      <img src={routeposition != "" && routeArray[routeposition - 2]  && carrierStore.findCarrierById(routeArray[routeposition - 2].carrierId) ? carrierStore.findCarrierById(routeArray[routeposition - 2].carrierId).pic : `https://avatars.dicebear.com/v2/identicon/unknown.svg` } className={style.secondNextCarrier} alt="person"></img>
    </div>
